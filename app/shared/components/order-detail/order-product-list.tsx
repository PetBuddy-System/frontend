  ...
  return (
    <div className='bg-card rounded-xl border border-border shadow-sm overflow-hidden'>
      <div className='px-6 py-4 border-b border-border flex items-center justify-between bg-muted/20'>
        <h2 className='font-bold text-base text-foreground'>{t('orderDetail.productsOrdered', 'Sản phẩm đã đặt')}</h2>
                ...
                <p className='text-xs text-muted-foreground'>
                  ...
                  {detail.salePrice != null && detail.salePrice < detail.unitPrice ? (
                    <>
                      ...
                      <span className='font-semibold text-foreground'>{formatPrice(detail.salePrice)}</span>
                    </>
                  ) : (
                    formatPrice(detail.unitPrice)
                  )}
