  ...
  return (
    <div className='bg-card p-6 rounded-xl border border-border shadow-sm'>
      <div className='flex items-center gap-2 mb-4 border-b border-border pb-3'>
        ...
        <h2 className='font-bold text-base text-foreground'>{t('orderDetail.paymentDetail', 'Chi tiết thanh toán')}</h2>
      </div>
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        ...
        <div className='bg-muted/40 p-4 rounded-xl border border-border/40 flex flex-col gap-2'>
          <div className='flex justify-between items-center text-xs'>
            <span className='text-muted-foreground'>{t('orderDetail.subtotal', 'Tổng giá trị sản phẩm')}</span>
            <span className='font-semibold text-foreground'>{formatPrice(subtotal)}</span>
          </div>
          <div className='flex justify-between items-center text-xs'>
            <span className='text-muted-foreground'>{t('orderDetail.shippingFee', 'Phí giao hàng')}</span>
            <span className='font-semibold text-foreground'>{formatPrice(shippingFee)}</span>
          </div>
          {discount > 0 && (
            <div className='flex justify-between items-center text-xs'>
              <div className='flex flex-col'>
                <span className='text-muted-foreground'>{t('orderDetail.voucherDiscount', 'Mã giảm giá')}</span>
                {order.voucherCode && (
          ...
          <div className='flex justify-between items-center'>
            <span className='font-bold text-sm'>{t('orderDetail.totalOrder', 'Tổng đơn hàng')}</span>
          ...
          {showPaidDeduction && (
            <>
              <div className='flex justify-between items-center text-xs'>
                <span className='text-muted-foreground'>{t('orderDetail.amountPaid', 'Số tiền đã thanh toán')}</span>
              ...
              <div className='h-px bg-border my-1' />
              <div className='flex justify-between items-center'>
                <span className='font-bold text-sm'>{t('orderDetail.amountToCollect', 'Số tiền cần thu')}</span>
                <span className='text-lg font-black text-emerald-600 dark:text-emerald-400'>{formatPrice(0)}</span>
              </div>
            </>
          )}
        </div>
