...
import serviceCombo from '../../assets/serviceCombo.png'
    ...
    <section className='w-full bg-muted py-20'>
      <div className='mx-auto max-w-6xl px-4 md:px-6'>
        <div className='mb-12 text-center'>
          <h2 className='font-display text-3xl font-bold text-primary md:text-4xl'>{t('combo.title')}</h2>
          <p className='mt-2 text-base text-muted-foreground'>{t('combo.subtitle')}</p>
        </div>

        <div className='grid grid-cols-1 gap-6 md:grid-cols-3'>
          {/* Promo image — desktop only */}
          <div className='hidden overflow-hidden rounded-2xl shadow-sm lg:block'>
            <img src={serviceCombo} alt={t('combo.promoImageAlt')} className='h-full w-full object-cover' />
          </div>
          ...
          <div className='grid grid-cols-1 gap-4 sm:grid-cols-3 md:col-span-3 lg:col-span-2'>
            {PACKAGES.map((pkg) => (
              ...
              >
                ...
                <MaterialIcon name={pkg.icon} filled className='text-4xl text-primary' />
                ...
                <p className='mb-4 text-sm text-muted-foreground'>{t(`combo.packages.${pkg.key}.description`)}</p>
                <button
                  type='button'
                  className={
                    pkg.featured
