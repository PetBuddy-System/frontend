    ...
    <section className='w-full bg-background py-20'>
      <div className='mx-auto max-w-6xl px-4 md:px-6'>
        <div className='mb-10 text-center'>
          <h2 className='font-display text-3xl font-bold text-primary md:text-4xl'>{t('priceBoard.title')}</h2>
        </div>
        ...
        <div className='overflow-hidden rounded-2xl border border-border/60 bg-card p-3 shadow-lg md:p-5'>
          <div className='flex min-h-[420px] items-center justify-center overflow-hidden rounded-xl border border-dashed border-border bg-muted md:min-h-[620px]'>
            {BangGia ? (
              <img src={BangGia} alt={t('priceBoard.imageAlt')} className='h-full w-full object-contain' />
            ) : (
              <div className='flex max-w-md flex-col items-center px-6 text-center'>
                ...
                <h3 className='font-display text-2xl font-bold text-foreground'>{t('priceBoard.placeholderTitle')}</h3>
