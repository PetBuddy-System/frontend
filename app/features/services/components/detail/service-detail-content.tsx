    ...
    <section className='mx-auto w-full max-w-6xl px-4 py-20 md:px-6'>
      ...
      <div className='grid grid-cols-1 gap-12 lg:grid-cols-3'>
        <div className='flex flex-col gap-8 leading-relaxed text-muted-foreground lg:col-span-2'>
          <div>
            <h2 className='mb-4 font-display text-xl font-semibold text-foreground'>{t('detail.process.title')}</h2>
          ...
          <div>
            <h2 className='mb-4 font-display text-xl font-semibold text-foreground'>{t('detail.benefits.title')}</h2>
            <div className='grid grid-cols-1 gap-6 md:grid-cols-3'>
              {BENEFITS.map((benefit) => (
                ...
                >
                  <MaterialIcon name={benefit.icon} className='mb-3 text-[32px] text-primary' />
                  <strong className='mb-2 text-sm font-semibold text-foreground'>
                    {t(`detail.benefits.items.${benefit.key}.title`)}
                  </strong>
                  <p className='text-sm'>{t(`detail.benefits.items.${benefit.key}.description`)}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              {PRICES.map((price) => (
                ...
                >
                  <div>
                    ...
                    <span className='text-sm text-muted-foreground'>{t(`detail.pricing.items.${price}.weight`)}</span>
                  </div>
