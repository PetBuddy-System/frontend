  ...
  return (
    <section className='mt-16 border-t border-border/60 pt-12'>
      <div className='flex flex-col items-start justify-between gap-4 md:flex-row md:items-center'>
        <div>
          <h2 className='text-2xl font-bold text-foreground font-display'>{t('gallery.title')}</h2>
          <p className='mt-2 text-sm text-muted-foreground'>{t('gallery.subtitle')}</p>
        </div>
        <button
          type='button'
      ...
      <div className='mt-8 grid h-[400px] grid-cols-2 gap-4 md:grid-cols-4'>
        ...
        >
          ...
          <span className='mt-2 text-sm font-semibold'>{t('gallery.share')}</span>
        </button>
      </div>
    </section>
  )
