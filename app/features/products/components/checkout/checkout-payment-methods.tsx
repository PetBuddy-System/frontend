  ...
  return (
    <section className='rounded-xl border border-border/60 bg-card p-6 shadow-sm md:p-8'>
          ...
          return (
            <label
              ...
              className={`flex cursor-pointer items-center gap-4 rounded-xl border p-4 transition-all ${
                isSelected ? 'border-primary bg-primary/5 ring-2 ring-primary/30' : 'border-border hover:bg-muted'
              }`}
            >
              <input
      ...
      {(selectedMethod === 'CARD' || selectedMethod === 'MOMO') &&
        user &&
        user.paymentFailStreak !== undefined &&
        user.paymentFailStreak >= 3 && (
          <div className='mt-4 flex items-start gap-3 rounded-xl border border-destructive/20 bg-destructive/5 p-4 text-sm text-destructive dark:border-destructive/30 dark:bg-destructive/10 animate-fade-in'>
            ...
            <div className='flex flex-col gap-1'>
              <span className='font-semibold'>{t('checkout.payment.warning.title')}</span>
