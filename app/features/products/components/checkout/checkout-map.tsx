...
export function CheckoutMap({ onLocationSelect, defaultLat = STORE_LAT, defaultLng = STORE_LON }: CheckoutMapProps) {
  ...
  useEffect(() => {
    ...
    marker.on('dragend', async () => {
      ...
      const valid = await validateLocation(position.lat, position.lng)
  ...
  // FIX: gọi BE validate vị trí (ngoài HCM / trên sông-hồ-biển) trước khi chấp nhận toạ độ.
  // BE đã trả sẵn message phù hợp qua ErrorCode (xem customFetch) -> chỉ cần hiển thị lại err.message
  async function validateLocation(lat: number, lng: number): Promise<boolean> {
  ...
  async function handleGetMyLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        const valid = await validateLocation(latitude, longitude)
        if (!valid) return
  ...
  async function handleSearchAddress(e: React.FormEvent) {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        const valid = await validateLocation(lat, lng)
    ...
    <div className='mt-6 rounded-xl border border-border/80 bg-muted/20 p-4 shadow-inner'>
      <div className='mb-3 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between'>
        ...
        <button
          ...
          className='flex items-center justify-center gap-1.5 rounded-lg border border-primary/30 bg-primary/5 px-3 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-primary hover:text-primary-foreground disabled:opacity-50'
        >
      ...
      <div className='relative h-64 w-full overflow-hidden rounded-xl border border-border bg-card'>
        ...
        {/* FIX: overlay loading khi đang gọi BE validate vị trí, tránh user thao tác chồng chéo */}
        {isValidating && (
