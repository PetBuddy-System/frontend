...
export function OrderSuccessBanner({ orderCode, email }: OrderSuccessBannerProps) {
    ...
    <section className='relative overflow-hidden rounded-3xl border border-border/60 bg-card p-8 text-center shadow-sm md:p-12'>
      ...
      <div className='relative'>
        ...
        <p className='mb-4 text-lg text-muted-foreground'>
          {t('orderSuccess.banner.orderCodePrefix')} <span className='font-bold text-foreground'>{orderCode}</span>
        </p>
        <div className='inline-block rounded-xl bg-muted px-6 py-3 text-sm font-semibold text-muted-foreground'>
          {t('orderSuccess.banner.emailPrefix')} <span className='font-bold text-primary'>{email}</span>
        </div>
      </div>
    </section>
  )
