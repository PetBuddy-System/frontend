    ...
    <section className='overflow-hidden rounded-3xl border border-border/60 bg-card shadow-sm'>
      <div className='flex items-center justify-between gap-4 border-b border-border px-6 py-5 md:px-8'>
        <h2 className='font-display text-2xl font-semibold text-foreground'>
          {t('orderSuccess.details.title', 'Chi tiết đơn hàng')}
        </h2>
          ...
          return (
            <article key={item.productId} className='flex items-center gap-5 md:gap-6'>
              ...
              <div className='min-w-0 flex-1'>
                <h3 className='line-clamp-2 font-display text-lg font-semibold text-foreground'>{item.name}</h3>
                <p className='mt-1 text-sm text-muted-foreground'>
                  {t('orderSuccess.details.quantity', { count: item.quantity })}
                </p>
                <div className='mt-1 flex items-center gap-2 text-sm'>
                  {hasSale && <span className='text-muted-foreground line-through'>{formatPrice(item.price)}</span>}
                  <span className={hasSale ? 'font-semibold text-primary' : 'text-muted-foreground'}>
                    {formatPrice(unitPrice)}
                  </span>
                </div>
              </div>
              <p className='text-right font-display text-lg font-bold text-primary'>{formatPrice(subtotal)}</p>
            </article>
          )
        })}
      </div>
