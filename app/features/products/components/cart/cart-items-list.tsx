    ...
    <div className='bg-card rounded-xl shadow-sm border border-border p-4'>
      <div className='flex gap-3'>
        ...
        <button
          ...
          disabled={isMutating}
          className='p-1.5 text-muted-foreground hover:text-destructive transition-colors rounded-full hover:bg-destructive/10 disabled:opacity-50 self-start'
        >
      ...
      <div className='flex items-center justify-between mt-4 pt-3 border-t border-border'>
        <div className='flex items-center gap-2'>
          <button
            type='button'
            className='w-8 h-8 flex items-center justify-center text-muted-foreground hover:bg-muted rounded-lg transition-colors disabled:opacity-50'
            onClick={() => onDecrease(item.key)}
            disabled={isMutating || item.quantity <= 1}
          >
            −
          </button>
          <span className='w-6 font-semibold text-foreground text-center text-sm'>{item.quantity}</span>
          <button
            type='button'
            className='w-8 h-8 flex items-center justify-center text-primary font-bold hover:bg-muted rounded-lg transition-colors disabled:opacity-50'
        ...
        <div className='text-right'>
          <p className='text-xs text-muted-foreground'>{t('cart.thanhtien')}</p>
          <p className='font-bold text-primary text-sm'>{formatPrice(effectiveSubtotal)}</p>
        </div>
      </div>
    </div>
  )
          ...
          <table className='w-full border-collapse'>
            <thead>
              <tr className='bg-muted text-muted-foreground text-sm font-semibold uppercase tracking-wider'>
                <th className='px-4 py-4 text-left'>{t('cart.sanpham')}</th>
                <th className='px-4 py-4 text-center'>{t('cart.dongia')}</th>
                <th className='px-4 py-4 text-center'>{t('cart.soluong')}</th>
                <th className='px-4 py-4 text-center'>{t('cart.thanhtien')}</th>
                <th className='px-4 py-4 text-center'>{t('cart.thaotac')}</th>
              </tr>
            </thead>
      ...
      <div className='lg:hidden space-y-3'>
        {items.map((item) => (
          <CartItemCard
            ...
            onRemove={onRemove}
            isMutating={mutatingItemId === item.key}
          />
        ))}
      </div>
    </div>
