    ...
    try {
      ...
      if (res.success) {
        if (isPaidByCard) {
          alert(
            t(
              'orderCancel.successMessagePaid',
        ...
        <main className='flex-1 flex items-center justify-center p-6'>
          <div className='flex flex-col items-center gap-3'>
            ...
            <p className='text-sm text-muted-foreground'>
              {t('orderCancel.loading', 'Đang tải thông tin đơn hàng...')}
            </p>
          </div>
        </main>
        <SiteFooter />
            ...
            <div className='lg:col-span-7 flex flex-col gap-6'>
                ...
                <div className='flex flex-col gap-4'>
                  {order.orderDetails?.map((item) => (
                    <div
                      ...
                      className='flex gap-4 p-4 bg-muted/50 rounded-lg border border-border/40'
                    >
                      ...
                      <div className='flex flex-col justify-between py-1 flex-grow min-w-0'>
                        <div>
                          <h3
                            className='text-foreground font-semibold text-sm md:text-base truncate'
                          ...
                          <div className='flex items-center gap-3 mt-1 text-muted-foreground text-xs font-medium'>
                            <span>
                              {t('orderCancel.quantity', {
                                count: item.quantity,
                                defaultValue: `Số lượng: x${item.quantity}`
                              })}
                            </span>
                          </div>
                        </div>
              ...
              <section className='bg-card border border-border/60 rounded-xl overflow-hidden shadow-sm'>
                <div className='p-6 border-b border-border/50 flex flex-col gap-3'>
                  ...
                  >
                    <option value=''>
                      {t('orderCancel.selectReasonPlaceholder', '-- Vui lòng chọn lý do hủy đơn --')}
                    </option>
                    <option value='Tôi không có nhu cầu mua nữa'>
                      {t('orderCancel.reasonNoNeed', 'Tôi không có nhu cầu mua nữa')}
                    </option>
                    <option value='Tôi muốn thay đổi địa chỉ nhận hàng'>
                      {t('orderCancel.reasonChangeAddress', 'Tôi muốn thay đổi địa chỉ nhận hàng')}
                    </option>
                    <option value='Tôi muốn đổi phương thức thanh toán'>
                      {t('orderCancel.reasonChangePayment', 'Tôi muốn đổi phương thức thanh toán')}
                    </option>
                    <option value='Tìm thấy giá rẻ hơn ở nơi khác'>
                      {t('orderCancel.reasonCheaperPrice', 'Tìm thấy giá rẻ hơn ở nơi khác')}
                    </option>
            ...
            <div className='lg:col-span-5 flex flex-col gap-6'>
              <section className='bg-card border border-border/60 rounded-xl p-6 shadow-sm flex flex-col'>
                <div className='flex items-center justify-between w-full mb-6'>
                  <h2 className='text-muted-foreground font-semibold text-xs uppercase tracking-wider'>
                    {t('orderCancel.refundAmountLabel', 'Số tiền hoàn lại')}
                  </h2>
