...
const MOCK_ENTRIES: TimelineEntry[] = [
  {
    ...
    description: 'Kiện hàng đã rời trung tâm phân loại TP.HCM và đang trên đường tới bưu cục phát.'
  },
  {
    icon: 'warehouse',
    time: '09:15 - 24/10/2024',
    title: 'Rời kho trung chuyển miền Nam',
    description: 'Đơn hàng đã được xử lý xong tại tổng kho và chuyển giao cho đối tác vận chuyển.'
  },
  {
    icon: 'box_edit',
  ...
  },
  {
    ...
    description: 'PetStore đã tiếp nhận và đang tiến hành đóng gói các sản phẩm của bạn.'
  }
]
            ...
            <div key={idx} className='relative flex items-start gap-8'>
              ...
              <div className='pl-14'>
                <div className='mb-1 flex flex-col gap-2 md:flex-row md:items-baseline'>
                  <time
                    className={cn('text-sm', isLatest ? 'font-bold text-primary' : 'font-medium text-muted-foreground')}
                  >
                    {entry.time}
                  </time>
                  <span className={cn('font-semibold', isLatest ? 'text-foreground' : 'text-muted-foreground')}>
                    {entry.title}
                  </span>
                </div>
