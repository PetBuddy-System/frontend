...
export function ViewReviewModal({
    ...
    // Lấy tên hiển thị
    const displayName = reviewData.anonymous
        ? 'Khách hàng'
        : reviewData.fullName || 'Người dùng'
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4'>
            <div className='relative w-full max-w-md rounded-2xl bg-background p-6 shadow-2xl animate-in fade-in zoom-in duration-200'>
                ...
                <div className='mt-3 flex items-center gap-2'>
                    <div className='flex gap-1 text-2xl'>
                        {[1, 2, 3, 4, 5].map((star) => (
                            <span
                                key={star}
                                className={star <= (reviewData.rating || 0)
                                    ? 'text-yellow-500'
                                    : 'text-muted'
                                }
                            >
                ...
                <div className='mt-3 flex items-center gap-2'>
                    ...
                    <span className={`text-sm font-medium px-2 py-0.5 rounded-full ${reviewData.status === 'ACTIVE'
