  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4'>
      <div className='relative w-full max-w-md rounded-2xl bg-background p-6 shadow-2xl'>
        ...
        <p className='mt-1 text-sm text-muted-foreground'>Chia sẻ trải nghiệm của bạn về đơn hàng này</p>
        ...
        <div className='mt-4 flex gap-2 justify-center'>
          {[1, 2, 3, 4, 5].map((star) => (
            ...
            >
              <span className={star <= (hoveredRating || rating) ? 'text-yellow-500' : 'text-muted'}>★</span>
            </button>
          ))}
        </div>
        <p className='mt-1 text-center text-sm text-muted-foreground'>{rating > 0 ? `${rating}/5` : 'Chọn số sao'}</p>
        ...
        <div className='mt-6 flex justify-end gap-2'>
          ...
          <Button onClick={handleSubmit} disabled={rating === 0 || !content.trim() || submitting}>
