    ...
    <section className='col-span-12 grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-5'>
      {STATS.map((stat) => (
        ...
        >
          ...
          <div>
            <p className='text-xs font-semibold text-muted-foreground md:text-sm'>{t(`stats.${stat.key}`)}</p>
            <p className='font-display text-lg font-bold text-primary md:text-xl'>{stat.value}</p>
          </div>
        </article>
      ))}
    </section>
