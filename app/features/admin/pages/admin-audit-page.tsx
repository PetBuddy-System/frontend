          ...
          <div className='mx-auto flex max-w-7xl flex-col gap-6'>
            ...
            <div className='flex flex-wrap items-end gap-3 rounded-xl border border-border bg-card p-3'>
              ...
              <div className='flex flex-col flex-1 min-w-[120px]'>
                <label className='text-xs font-medium text-muted-foreground mb-1'>{t('audit.filter.entityType')}</label>
                <select
              ...
              <div className='flex flex-col flex-1 min-w-[100px]'>
                <label className='text-xs font-medium text-muted-foreground mb-1'>{t('audit.filter.action')}</label>
                <select
                  value={filter.action || ''}
              ...
              {/* Entity Code */}
              <div className='flex flex-col flex-1 min-w-[120px]'>
                <label className='text-xs font-medium text-muted-foreground mb-1'>{t('audit.filter.entityCode')}</label>
                <input
                  type='text'
                      ...
                      {logs.length === 0 ? (
                        ...
                        logs.map((log) => (
                          <tr key={log.id} className='transition-colors hover:bg-muted/30'>
                            ...
                            <td className='px-4 py-3 text-sm whitespace-nowrap'>{log.performedBy}</td>
                            ...
                            <td className='px-4 py-3 text-sm font-mono'>{log.entityCode || '--'}</td>
                            <td className='px-4 py-3 text-sm text-muted-foreground'>{getReasonText(log.reason, t)}</td>
                ...
                {totalPages > 0 && (
                  <div className='flex items-center justify-between border-t border-border px-4 py-3'>
                    <span className='text-sm text-muted-foreground'>
                      {t('audit.pagination.showing')} {filter.page! * filter.size! + 1} -{' '}
                      {Math.min((filter.page! + 1) * filter.size!, totalElements)} {t('audit.pagination.of')}{' '}
                      {totalElements}
                    </span>
                    <div className='flex items-center gap-1'>
                      <button
                        ...
                        className={cn(
                          ...
                          filter.page === 0 ? 'text-muted-foreground/50 cursor-not-allowed' : 'hover:bg-muted'
                        )}
                      >
