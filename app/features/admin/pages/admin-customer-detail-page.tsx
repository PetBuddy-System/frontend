...
export function AdminCustomerDetailPage() {
  ...
  const [message, setMessage] = useState<{ type: 'error' | 'success'; text: string } | null>(() => {
    ...
    if (state && typeof state === 'object' && 'customerUpdated' in state && state.customerUpdated === true) {
      return { type: 'success', text: t('customers.messages.updateSuccess') }
    }
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Customer detail is loaded from route params.
    void loadUser()
  }, [loadUser])
  ...
  return (
    <div className='flex h-screen overflow-hidden bg-background text-foreground'>
      ...
      <div className='flex min-w-0 flex-1 flex-col overflow-hidden'>
        <AdminTopNav titleKey='customers.detail.pageTitle' subtitleKey='customers.detail.pageSubtitle' />
          ...
          <div className='mx-auto flex max-w-5xl flex-col gap-6'>
            <section className='flex flex-col gap-4 border-b border-border pb-6 md:flex-row md:items-center md:justify-between'>
              <div>
                ...
                <p className='mt-2 text-muted-foreground'>{t('customers.detail.subtitle')}</p>
              </div>
              <div className='flex flex-wrap gap-3'>
                ...
                {user ? (
                  <Button type='button' onClick={() => void navigate(`/admin/employees/${user.userId}/edit`)}>
            ...
            {isLoading ? (
              ...
              <section className='rounded-xl border border-border bg-card p-5 shadow-sm'>
                <div className='flex flex-col gap-5 border-b border-border pb-5 md:flex-row md:items-center md:justify-between'>
                  <div className='flex items-center gap-4'>
                    {avatarUrl ? (
                      <img
                        src={avatarUrl}
                        alt={t('customers.detail.avatarAlt')}
                ...
                <div className='mt-5 grid gap-3 md:grid-cols-2'>
                  <DetailItem
                    icon='wc'
                    label={t('customers.detail.fields.gender')}
                    value={t(`users.gender.${normalizeAdminGender(user.gender)}`, { defaultValue: user.gender || '-' })}
                  />
                  <DetailItem
                    icon='cake'
                    label={t('customers.detail.fields.dateOfBirth')}
