            ...
            <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
              <div>
                ...
                <p className='text-muted-foreground'>
                  {t(
                    'shippingSubtitle',
                    'Thiết lập biểu phí dựa trên khoảng cách địa lý để tối ưu hóa chi phí vận hành.'
                  )}
                </p>
              </div>
            </div>
            ...
            <div className='overflow-hidden rounded-xl border border-border/60 bg-card shadow-sm'>
              ...
              <div className='overflow-x-auto'>
                {isLoading ? (
                  ...
                  <div className='py-12 text-center text-muted-foreground'>Không tìm thấy quy tắc vận chuyển nào.</div>
                ) : (
                  <table className='w-full border-collapse text-left'>
                    <thead>
                      <tr className='bg-muted/50 border-b border-border/60'>
                        ...
                        <th className='px-6 py-4 font-semibold text-muted-foreground text-sm text-right w-28'>
                          Thao tác
                        </th>
                      </tr>
                    </thead>
                    <tbody className='divide-y divide-border/40'>
                      {filteredRules.map((rule) => (
                        <tr key={rule.id} className='hover:bg-muted/30 transition-colors'>
                          ...
                          <td
                            className={`px-6 py-4 text-sm font-bold ${rule.fee === 0 ? 'text-success' : 'text-primary'}`}
                          >
