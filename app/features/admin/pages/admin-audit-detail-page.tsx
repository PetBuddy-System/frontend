            ...
            return (
                <div className="rounded-lg bg-red-50 p-3 dark:bg-red-950/20">
                    <p className="text-sm font-medium text-red-700 dark:text-red-400">
                        ❌ {t('audit.actions.REMOVED')}
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {oldData.name} ({oldData.code})
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {t('audit.fields.promotionType')}: {getPromotionTypeLabel(oldData.type, t)} | {t('audit.fields.discountValue')}: {oldData.value}
                    </p>
                </div>
            )
        }
            ...
            return (
                <div className="rounded-lg bg-green-50 p-3 dark:bg-green-950/20">
                    <p className="text-sm font-medium text-green-700 dark:text-green-400">
                        ✅ {t('audit.actions.ADDED')}
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {newData.name} ({newData.code})
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {t('audit.fields.promotionType')}: {getPromotionTypeLabel(newData.type, t)} | {t('audit.fields.discountValue')}: {newData.value}
                    </p>
                </div>
            )
        }
            ...
            return (
                <div className="rounded-lg bg-blue-50 p-3 dark:bg-blue-950/20">
                    <p className="text-sm font-medium text-blue-700 dark:text-blue-400">
                        🔄 {t('audit.actions.UPDATED')}
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {oldData.name} ({oldData.code})
                    </p>
                    <p className="text-sm text-muted-foreground">
                        {t('audit.fields.promotionType')}: {getPromotionTypeLabel(oldData.type, t)} → {getPromotionTypeLabel(newData.type, t)} | {t('audit.fields.discountValue')}: {oldData.value} → {newData.value}
                    </p>
                </div>
            )
        }
        ...
        <div className="flex items-start gap-3 rounded-lg bg-muted/30 p-3">
            ...
            <div className="flex flex-col gap-1">
                {isCreate ? (
                    ...
                    <>
                        <span className="text-sm text-muted-foreground line-through">
                            {change.oldValue ? (change.field === 'startDate' || change.field === 'endDate' ? formatDateDisplay(change.oldValue) : change.oldValue) : 'null'}
                        </span>
                        <span className="text-sm text-muted-foreground">↓</span>
