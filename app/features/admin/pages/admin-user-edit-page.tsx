...
export function AdminUserEditPage() {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- User edit data is loaded from route params.
    void loadUser()
  }, [loadUser])
  ...
  async function handleSubmit(payload: AdminEmployeeCreatePayload | AdminEmployeeUpdatePayload, avatar: File | null) {
    ...
    try {
      await adminUsersApi.updateUser(userId, payload as AdminEmployeeUpdatePayload, avatar)
      setMessage({ type: 'success', text: t('users.messages.updateSuccess') })
      void navigate(`/admin/users/${userId}`, { state: { employeeUpdated: true } })
    } catch (error) {
      setMessage({
        type: 'error',
        text: error instanceof Error ? error.message : t('users.messages.updateFailed')
      })
    } finally {
  ...
  return (
    <div className='flex h-screen overflow-hidden bg-background text-foreground'>
      ...
      <div className='flex min-w-0 flex-1 flex-col overflow-hidden'>
        <AdminTopNav titleKey='users.edit.pageTitle' subtitleKey='users.edit.pageSubtitle' />
          ...
          <div className='mx-auto flex max-w-4xl flex-col gap-6'>
            <section className='flex flex-col gap-4 border-b border-border pb-6 md:flex-row md:items-center md:justify-between'>
              <div>
                ...
                <p className='mt-2 text-muted-foreground'>{t('users.edit.subtitle')}</p>
              </div>
            ...
            <section className='rounded-xl border border-border bg-card p-5 shadow-sm'>
              {isLoading ? (
                ...
                <AdminEmployeeForm
                  ...
                  onCancel={() => void navigate(`/admin/users/${user.userId}`)}
                  onSubmit={handleSubmit}
                />
              ) : (
                <div className='py-8 text-center text-sm text-muted-foreground'>{t('users.messages.notFound')}</div>
              )}
            </section>
          </div>
        </main>
