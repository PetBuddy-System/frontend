import type { StaffAttendanceStatus, StaffScheduleShiftType, StaffScheduleStatus } from '../services'
...
export function getStaffAttendanceStatusClassName(status: StaffAttendanceStatus) {
  if (status === 'ON_TIME') return 'bg-success/10 text-success border-success/20'
  if (status === 'LATE') return 'bg-warning/10 text-warning border-warning/20'
  if (status === 'LEAVE') return 'bg-info/10 text-info border-info/20'
