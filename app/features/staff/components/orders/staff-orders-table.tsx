...
export function StaffOrdersTable({
  ...
  function renderStatusBadge(status: string) {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold uppercase ${badgeStyle}`}
      >
  ...
  function renderPaymentStatusBadge(status?: string) {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase w-fit ${style}`}
      >
        {label}
      </span>
    )
                  ...
                  <td className='px-6 py-4 text-right'>
                    <div className='flex items-center justify-end gap-2 flex-wrap'>
                      ...
                      {order.status === 'CANCEL_REQUESTED' && isCoordinator && (
                        <button
                          onClick={async () => {
                            if (!window.confirm(t('staffOrdersTable.confirm.refundPrompt', { code: order.orderCode })))
                              return
                            try {
