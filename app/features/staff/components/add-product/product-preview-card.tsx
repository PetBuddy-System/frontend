  ...
  return (
    ...
    >
      ...
      <div className='mb-2 flex items-center gap-2'>
        ...
        <span className='text-xs text-muted-foreground sm:text-sm'>{t('addProduct.preview.indexLabel')}</span>
      </div>
      ...
      <div className='space-y-1.5 sm:space-y-2'>
        ...
        <div className='flex flex-wrap items-center gap-1.5 sm:gap-2'>
          {categoryLabel && (
            <span className='rounded-full bg-primary/10 px-2 py-0.5 text-xs text-primary'>{categoryLabel}</span>
          )}
          {product.supplier && (
            <span className='rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground'>{product.supplier}</span>
          )}
        </div>
        ...
        <div className='flex items-center justify-between pt-1'>
          {product.price ? (
            <span className='font-mono text-base font-bold text-primary sm:text-lg'>{formatPrice(product.price)}đ</span>
          ) : (
            <span className='text-xs text-muted-foreground sm:text-sm'>{t('addProduct.preview.noPrice')}</span>
          )}
          {product.stock && (
