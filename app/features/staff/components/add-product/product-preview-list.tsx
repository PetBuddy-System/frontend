  ...
  if (products.length === 0) {
    return (
      <div className='flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-muted-foreground/20 bg-muted/30 p-6 text-center sm:p-8'>
        ...
        <p className='text-sm font-medium text-muted-foreground'>{t('addProduct.preview.empty')}</p>
        <p className='mt-1 text-xs text-muted-foreground/70'>{t('addProduct.preview.emptyHint')}</p>
      </div>
    )
  }
  ...
  return (
    <div className='space-y-3 sm:space-y-4'>
      ...
      <div className='flex items-center justify-between'>
        <h3 className='text-base font-bold text-primary sm:text-lg'>{t('addProduct.preview.title')}</h3>
      ...
      <div className='grid grid-cols-1 gap-2 sm:gap-3'>
        {products.map((product, index) => (
          <ProductPreviewCard key={product.id} product={product} index={index} onRemove={onRemove} />
        ))}
      </div>
    </div>
  )
