  ...
  const loadLogs = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        const filtered = res.data.content.filter((log) => ALLOWED_ENTITY_TYPES.has(log.entityType as string))
        setLogs(filtered)
                      ...
                      {logs.length === 0 ? (
                        ...
                        logs.map((log) => (
                          <tr key={log.id} className='transition-colors hover:bg-muted/30'>
                            ...
                            <td className='whitespace-nowrap px-4 py-3 text-sm'>{log.performedBy}</td>
                ...
                {totalPages > 1 && (
                  <div className='flex items-center justify-between border-t border-border px-4 py-3'>
                    <span className='text-sm text-muted-foreground'>
                      {t('auditLogs.pagination.showing', 'Hiển thị')} {filter.page! * filter.size! + 1} -{' '}
                    ...
                    <div className='flex items-center gap-1'>
                      <button
                        ...
                        className={cn(
                          ...
                          filter.page === 0 ? 'cursor-not-allowed text-muted-foreground/50' : 'hover:bg-muted'
                        )}
                      >
