        ...
        <main className='flex-1 overflow-y-auto p-4 md:p-6'>
          <div className='mx-auto flex max-w-7xl flex-col gap-6'>
            <section className='flex flex-col gap-2 border-b border-border pb-4'>
              ...
              <h1 className='font-display text-2xl font-bold text-card-foreground md:text-3xl'>{t('restock.title')}</h1>
              <p className='text-muted-foreground text-sm'>{t('restock.subtitle')}</p>
            </section>

            <section className='overflow-hidden rounded-2xl border border-border bg-card shadow-sm'>
              <RestockList key={refreshKey} onSelectReturn={handleSelectReturn} onViewDetail={handleViewDetail} />
            </section>
          </div>
        </main>
      </div>
