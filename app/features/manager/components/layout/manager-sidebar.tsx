...
const MANAGER_NAV_ITEMS = [
  ...
  { icon: 'history', key: 'auditLogs', href: '/manager/audit-logs' },
] as const
  ...
  return (
    <>
      ...
      >
        ...
        <nav className='flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto'>{renderNavItems()}</nav>

        <div className='my-3 h-px shrink-0 bg-border' />
        <nav className='flex shrink-0 flex-col gap-1'>{renderLogoutButton()}</nav>
      </aside>
    </>
  )
}
